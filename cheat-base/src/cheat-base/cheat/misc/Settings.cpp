...
namespace cheat::feature
{
	Settings::Settings() : Feature(),
		...
		NF(f_FontSize, u8"字体大小", "General::Theme", 16.0f),
		NF(f_ShowStyleEditor, u8"显示颜色自定义", "General::Theme", false),
		NFS(f_DefaultTheme, u8"主题", "General::Theme", ""),
	...
	void Settings::DrawMain()
	{
		...
		ImGui::BeginGroupPanel(u8"自定义主题");
		{
			...
			ImGui::SetNextItemWidth(200);
		    ImGui::InputText(u8"主题名字", &themeNameBuffer_);

			bool alreadyExist = m_Themes.count(themeNameBuffer_) > 0;

			ImGui::SameLine();
			if (ImGui::Button(alreadyExist ? u8"替代" : u8"保存"))
			{
				ImGui::SameLine();
				if (themeNameBuffer_.empty())
					ImGui::Text(u8"主题名称无效.回退到默认主题.");
			...
			ImGui::SetNextItemWidth(200);
			if (ImGui::BeginCombo(u8"主题选择", f_DefaultTheme.value().c_str()))
			{
			...
			ImGui::SameLine();
			if (ImGui::Button(u8"删除主题"))
			{
				std::filesystem::remove(themesDir / (f_DefaultTheme.value() + ".json"));
				LOG_INFO(u8"删除主题 %s.", f_DefaultTheme.value().c_str());
				f_DefaultTheme = "";
				hasLoaded = false;
				Init();
			}

			ConfigWidget(f_ShowStyleEditor, u8"显示 ImGui 主题自定义窗口.");
		}
		ImGui::EndGroupPanel();
	}
