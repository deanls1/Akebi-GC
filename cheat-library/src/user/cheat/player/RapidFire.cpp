...
namespace cheat::feature
{
	...
	RapidFire::RapidFire() : Feature(),
        ...
        NF(f_AnimationMultiplier, u8"动画倍数",  u8"攻击模块", 100),
        NF(f_AnimationState, u8"动画状态",  u8"攻击模块", 0.5f),
        NF(f_AttackSpeed, u8"攻击速度",  u8"攻击模块", false),
        NF(f_SpeedMultiplier, u8"速度乘数",  u8"攻击模块", 1.5f),
        animationCounter(1)
        {
	...
	void RapidFire::DrawMain()
	{
		...
		ConfigWidget(u8"多倍技能", f_MultiAnimation, u8"启用多动画攻击.\n" \
			u8"请记住，角色的音频也会被重复.");
		ConfigWidget(u8"动画倍数", f_AnimationMultiplier, 1, 1, 150, u8"配置更新动画状态的次数.\n" \
			u8"结果可能会随着动画状态而变化.");
		ConfigWidget(u8"动画状态", f_AnimationState, 0.01f, 0.f, 2.f, u8"要重播的动画状态.\n"\
			u8"结果可能与动画乘数一起变化.");
		ConfigWidget(u8"攻击速度", f_AttackSpeed, u8"启用快速动画攻击.\n");
		ConfigWidget(u8"速度乘数", f_SpeedMultiplier, 0.1f, 1.0f, 5.0f, u8"攻击速度倍增.");
	}
