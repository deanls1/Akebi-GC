...
#undef printString
    ...
    void DrawEntityGroupActionButtons(std::vector<game::Entity*> entities, bool& csvFriendly, bool& includeHeaders)
    {
        ...
        if (csvFriendly) {
            ImGui::SameLine();
            ImGui::Checkbox(u8"包括 Headers", &includeHeaders);
            if (ImGui::IsItemHovered())
    ...
    static void DrawEntitiesData()
    {
        ...
        if (ImGui::BeginTabBar("EntityManagerTabBar", tab_bar_flags))
        {
            ...
            if (ImGui::BeginTabItem(u8"Entity 列表"))
            {
...
#undef DRAW_BOOL
    ...
    void Debug::DrawMain()
    {
        if (ImGui::CollapsingHeader(u8"实体管理", ImGuiTreeNodeFlags_None))
            DrawEntitiesData();
        ...
        if (ImGui::CollapsingHeader(u8"地图管理", ImGuiTreeNodeFlags_None))
            DrawManagerData();
